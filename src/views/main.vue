<template>
  <main class="maincontents">
    ...
    <section class="newbbok">
      <h1 class="maintit">
        새로 나온 도서<span>분야별 새로 나온 도서를 만나 보세요</span>
      </h1>
    ...
    <section class="searchbook">
      <h1 class="maintit">
        도서검색<span>찾고자 하는 도서명을 검색해 주세요</span>
      </h1>
      <div class="inputarea">
        ...
        >
          ...
          <div class="nonemessage" v-else>
            <i class="bi bi-x-circle-fill" />검색 결과가 없습니다.
          </div>
        </div>
      </div>
      <div class="guidehash">
  ...
  computed: {
    autocomplate() {
      const resultlists = this.booksname.filter((item) => {
        if (item.cata.match(this.keyword)) return item;
      });
      return resultlists;
    },
  },
